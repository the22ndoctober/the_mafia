...
export default function Player({slot,name,color,showRoles,isVote}){
    const dispatch = useDispatch()
    const active = useSelector(selectActive)
    const statuses = useSelector(selectStatus)
    const exhibation = useSelector(selectExhibition)
    const didNotVoted = useSelector(selectVote)
    ...
    <div className="player__row" >
        ...
        <div className={`player__background ${active === slot ? 'player__background-active' : ''}` } style={{backgroundColor: color}} >    
            ...
            <div className="player__name" onClick={isVote? voteForSMBD : beExposed}>{`${slot+1}.`} {name}</div>
            <Fouls id={slot}/>   
        </div>
        <Roles showRoles={showRoles} id={slot}/>    
        <div className={`player__exposed ${exposed !== 'none'? '' : 'player__exposed-hidden'}`} style={{backgroundColor: enums.playerColors[exposed]}} onClick={isVote? ()=>{} : remove} >{exposed !== 'none'? exposed + 1 + '.' : ''}</div>
     </div>
     )
